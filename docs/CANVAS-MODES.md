...
> **【锁定声明 2026-09-02】当前锁定方案 C（Scale.RESIZE），`CANVAS_MODE = 'c'`。**
> 用户决策：画布 CSS 尺寸必须与视口一致（小米设备 2340×1080，19.5:9 铺满无黑边），
> 原方案 D「判定区比例 FIT 保护」红线由用户主动放开。
> - 唯一权威配置：`src/config/CanvasMode.ts` 的 `CANVAS_MODE` 常量，禁止运行时改写画布尺寸/样式；
> - 回归门禁：`npm run verify:canvas-lock`（CDP 模拟小米 2340×1080，断言 canvas CSS=物理=视口，6 项断言全过才算锁定完好）；
> - 发布方式：`./scripts/deploy.sh base`（重建 dist-c/，5173/5180 自动读新包）；
> - 下文所述「默认方案 D」已过时，以本声明为准。
