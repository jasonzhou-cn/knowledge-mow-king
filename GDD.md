...
### 2.2 答题交互设计回顾（箭头选择 + 移动轨道，双模式可配置）

**设计初衷**：默认采用「**固定答案 + 箭头循环选择**」模式——4 个答案固定不动，一个高亮箭头（金色描边 + 顶部 ▼）在选项间循环跳转，玩家在目标选项高亮时点击 / 空格确认。此设计降低儿童认知负荷：眼睛只需追踪一个移动元素，而不是同时盯 4 个移动卡片；同时保留「移动答案 + 停住判定」模式作为可配置回退（`answerSettings.mode = "track"`），照顾偏好旧玩法的玩家。

**两种模式的具体形态**：

* **arrow（默认）**：4 个选项固定显示在一行 / 2×2 网格（`arrowLayout`），高亮箭头按 `arrowInterval`（默认 1.2 秒）循环跳转；点击 / 空格 = 确认当前高亮选项，箭头指向正确答案即答对。没有「未命中」概念（确认即选中），`missCount` 恒 0。
* **track**：选项沿轨道匀速移动（圆 / 线性），玩家点击停住，按卡片与判定区的重叠面积判定；未命中（重叠比例 < 阈值）不消耗题数，可重试（`missCount` 累加）。
...
* 操作难度与答题难度失衡（track 模式）：玩家明明知道正确答案，但就是点不中移动速度过快的目标，会把游戏失败归因于操作而非知识储备，产生强烈的挫败感。
* 移动速度设计不合规（track 模式）：没有依据年龄段的反应能力设置速度区间，或速度变化的梯度设计不合理，导致部分玩家群体从一开始就无法完成操作。
* 高亮节奏过快（arrow 模式）：`arrowInterval` 过短会导致小朋友来不及反应等同失衡，但过长会变得无挑战。
* 视觉反馈缺失：玩家无法精准判断点击 / 高亮的时机，或没有明显的命中 / 未命中反馈，导致操作体验混沌。

  **最佳实践验证**：

* 难度分层适配：track 模式按年龄段分别设置移动速度、目标悬浮大小、判定区域参数；arrow 模式按年龄段设置 `arrowInterval` 与卡片尺寸。完全符合各年龄段玩家的平均认知水平。
* 节奏绑定逻辑合规：选项移动速度 / 高亮跳转节奏仅与玩家等级和关卡难度绑定，与答题正确率、割草表现无任何直接关联；不会出现「答题错误导致节奏加快」的双重惩罚。
* 操作容错保障合规：track 模式参考《Stop the Cloud》的核心设计逻辑，设置合理重叠阈值；arrow 模式则直接消除操作维度（不要求精准点击），把认知资源留给「认识正确答案」本身。
* 检测机制合理：track 模式设有移动速度下限 / 上限；arrow 模式设有 `arrowInterval` 上下限（0.4s~4s），保证两端玩家都能接受。

  **结论**：答题交互设计（arrow 默认 + track 可选）完全符合适龄设计规范，没有不可控的体验风险。
...
* 多维度增量设计合规：设计要求每关的难度变化必须控制在 30% 以内，且只能有一个主升维度，例如第 1 关仅提升小怪血量、第 2 关仅增加小怪刷新数量、第 3 关提升答题节奏（缩短答题时间 / 加快箭头跳转间隔），不会出现多维度同时增量的情况。
